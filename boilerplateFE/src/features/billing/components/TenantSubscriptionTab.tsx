...
import { useTenantSubscription, useTenantUsage, useTenantPayments, useChangeTenantPlan, usePlans } from '@/features/billing/api';
...
export function TenantSubscriptionTab({ tenantId, tenantName }: TenantSubscriptionTabProps) {
  ...
  const handleChangePlan = () => {
    if (tenantId && selectedPlanId) {
      changePlan(
        { tenantId, data: { planId: selectedPlanId, interval: selectedInterval } },
        { onSuccess: () => { setPlanModalOpen(false); setSelectedPlanId(''); } },
      );
    }
  };
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-lg font-semibold text-foreground">
        {t('billing.subscriptionFor', { name: tenantName })}
      </h2>
      ...
      <section className="space-y-3">
        ...
        {subLoading ? (
          ...
          <EmptyState icon={CreditCard} title={t('billing.currentPlan')} description={t('billing.subtitle')} />
        ) : (
          <Card>
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2">
                    <h4 className="text-lg font-bold text-foreground">{subscription.planName}</h4>
        ...
        {usageLoading ? (
          ...
          <Card>
            <CardContent className="py-5 grid gap-5 sm:grid-cols-2">
              <UsageBar label={t('billing.usersUsage')} current={usage.users} max={usage.maxUsers} />
              <UsageBar label={t('billing.storageUsage')} current={usage.storageBytes} max={usage.maxStorageBytes} formatValue={formatFileSize} />
              <UsageBar label={t('billing.apiKeysUsage')} current={usage.apiKeys} max={usage.maxApiKeys} />
              <UsageBar label={t('billing.usage') + ' — Reports'} current={usage.reportsActive} max={usage.maxReports} />
            </CardContent>
          </Card>
        )}
      </section>
      ...
      {/* Payment History */}
      <section className="space-y-3">
        <h3 className="text-base font-semibold text-foreground">{t('billing.payments')}</h3>
        {paymentsLoading ? (
          <div className="flex justify-center py-8"><Spinner size="lg" /></div>
        ) : payments.length === 0 ? (
            ...
            <TableBody>
              {payments.map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell className="font-medium text-foreground">{p.currency} {p.amount.toFixed(2)}</TableCell>
                  <TableCell>
      ...
      <Dialog open={planModalOpen} onOpenChange={setPlanModalOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>{subscription ? t('billing.changePlanConfirmDesc') : ''}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <Select value={String(selectedInterval)} onValueChange={(v) => setSelectedInterval(Number(v))}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="0">{t('billing.monthly')}</SelectItem>
                <SelectItem value="1">{t('billing.annual')}</SelectItem>
              </SelectContent>
            </Select>
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setPlanModalOpen(false)}>{t('common.cancel')}</Button>
              <Button onClick={handleChangePlan} disabled={!selectedPlanId || selectedPlanId === subscription?.subscriptionPlanId || isChanging}>
