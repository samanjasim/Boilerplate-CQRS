...
interface ActivityBreakdownChartProps {
  actionData: TimeSeriesPoint[];
  entityData: TimeSeriesPoint[];
}
...
export function ActivityBreakdownChart({ actionData, entityData }: ActivityBreakdownChartProps) {
  const { t } = useTranslation();

  const topActions = actionData.slice(0, 5);
  const topEntities = entityData.slice(0, 5);

  const hasData = topActions.length > 0 || topEntities.length > 0;
