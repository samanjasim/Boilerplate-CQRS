...
**Ask: "Does core code reference this entity?"**

| Answer | Where entity lives | Example |
|--------|-------------------|---------|
| **Yes** — a core interface, handler, interceptor, or service uses it | `Starter.Domain` (core) | `FileMetadata` — used by `IFileService` which Tenants/Reports need |
| **No** — only the module's own handlers/controllers use it | Inside the module's `Domain/` folder | A future `Product` entity for an E-commerce module |

**The compile test:** If you delete the module's folder and the project still compiles, the entity belongs in the module. If it breaks core code, it belongs in core.

**Promotion rule:** If an entity starts in a module but later 3+ modules depend on it, promote it to `Starter.Domain`. This should be rare — it means the entity has become foundational.

#### Current entities — where they belong

| Entity | Core references it? | Lives in |
|--------|-------------------|----------|
| `User`, `Role`, `Permission`, `Tenant` | Yes — auth, identity, multi-tenancy | Core Domain |
| `FileMetadata` | Yes — `IFileService` returns it, Tenants use it for branding | Core Domain |
| `AuditLog` | Yes — `AuditableEntityInterceptor` creates them | Core Domain |
| `SystemSetting` | Yes — `ISettingsService` uses it | Core Domain |
| `FeatureFlag` | Yes — `IFeatureFlagService` uses it | Core Domain |
| `Notification` | Yes — core email/notification pipeline references it | Core Domain |
| `SubscriptionPlan` | Borderline — `IBillingProvider` references it | Core Domain |
| `ApiKey` | No — only ApiKeys handlers use it | Could be in module |
| `WebhookEndpoint` | No — only Webhook handlers use it | Could be in module |
| `ImportJob` | No — only ImportExport handlers use it | Could be in module |

#### Module-only entity example

A future E-commerce module's entities live entirely inside the module:

```
modules/Starter.Module.Ecommerce/
├── Domain/
│   ├── Entities/
│   │   ├── Product.cs          ← not referenced by core
│   │   ├── Category.cs
│   │   ├── Order.cs
│   │   └── CartItem.cs
│   └── Enums/
...
├── Infrastructure/
│   └── Configurations/
│       ├── ProductConfiguration.cs   ← auto-discovered by DbContext
│       └── OrderConfiguration.cs
```

Remove the module → entities, tables, and endpoints all disappear. Core compiles unchanged.

### Cross-Module Dependencies

When Module A needs something from Module B, pick based on coupling strength:

#### Option 1: Direct module dependency (strong coupling)

Module A declares a dependency on Module B. It can reference B's entities and services directly.

```csharp
// DeliveryModule.cs
public IReadOnlyList<string> Dependencies => ["Starter.Module.Ecommerce"];
```

`ModuleLoader.ResolveOrder()` ensures Ecommerce loads before Delivery. The CLI `starter module remove Ecommerce` warns that Delivery depends on it.

**When to use:** Delivery fundamentally can't exist without Orders.

#### Option 2: Domain events (loose coupling)

Module A raises a MediatR notification. Module B handles it if present.

```csharp
// E-commerce module raises event (no dependency on Delivery)
RaiseDomainEvent(new OrderPlacedEvent(orderId, tenantId, deliveryAddress));

// Delivery module handles it (if installed — if not, event fires silently)
public class ScheduleDeliveryHandler : INotificationHandler<OrderPlacedEvent> { ... }
```

**When to use:** Delivery reacts to something that happened but doesn't need to query Orders directly.

#### Option 3: Core interface (shared service)

If multiple modules need the same capability, define the interface in `Starter.Application` and let any module inject it. The implementation lives in Infrastructure or a specific module.

```csharp
// Core interface — any module can inject it
...
**When to use:** The service is a platform capability, not module-specific logic.

#### Decision matrix

| Scenario | Solution |
|----------|----------|
| Module uses a core service (`IFileService`, `ICacheService`) | Inject directly — always available |
| Module queries another module's entity | Declare module dependency, use `_context.Set<T>()` |
| Module reacts to something in another module | Domain events via MediatR `INotificationHandler<T>` |
| Multiple modules need the same capability | Define interface in core `Starter.Application` |
| Hard dependency — Module A can't exist without B | `IModule.Dependencies`, CLI warns on removal |

### Module Structure Template

Every module follows the same layout. All are single .csproj projects:

```
modules/Starter.Module.{Name}/
├── Starter.Module.{Name}.csproj      ← references Module.Abstractions only
├── {Name}Module.cs                   ← implements IModule
├── Constants/
│   └── {Name}Permissions.cs          ← static permission constants
├── Domain/                           ← ONLY if entity is module-only
│   ├── Entities/
│   └── Enums/
├── Application/
│   ├── Commands/{Action}/
...
│   └── DTOs/{Name}Dto.cs
├── Infrastructure/
│   ├── Configurations/               ← EF configs for module entities
│   └── Services/                     ← module-specific service implementations
└── Controllers/
    └── {Name}Controller.cs
```
