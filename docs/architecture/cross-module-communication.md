...
5. Consumers implement `IConsumer<MyEvent>` anywhere — but each module that ships consumers must opt them into the bus by implementing `IModuleBusContributor` (in `Starter.Abstractions.Messaging`) and calling `bus.AddConsumers(typeof(MyModule).Assembly)`. The host (`Starter.Infrastructure.DependencyInjection`) wires the core assembly automatically and iterates each module's `ConfigureBus` callback; it stopped auto-scanning module assemblies in Tier 2.5 Theme 5. The architecture test `ModuleRegistryTests.Modules_with_MassTransit_consumers_implement_IModuleBusContributor` fails the build if a module ships a consumer without declaring the contract.
