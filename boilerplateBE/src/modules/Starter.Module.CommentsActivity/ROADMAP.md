...
Integrator-facing integration docs live in [`DEVELOPER_GUIDE.md`](./DEVELOPER_GUIDE.md). This file is maintainer-facing.
...
### Write-side tenant hardening for edit/delete

**What:** Apply the same `ResolveEffectiveTenantIdAsync` preamble that `AddCommentAsync` / `RecordAsync` / `WatchAsync` got in this bundle to `EditCommentAsync` and `DeleteCommentAsync`. Today the edit/delete paths trust the comment's persisted `TenantId`, which means a caller with the raw comment id can act cross-tenant.

**Why deferred:** The create-path hole was the one observed during multi-tenant gate testing; edit/delete require the caller to already possess a comment id from the target tenant, which is harder to reach by accident. Shipping the broader hardening in one bundle kept the scope tight and the diff reviewable.

**Pick this up when:** A cross-tenant edit or delete surfaces in testing, OR preemptively the next time this module is opened for a bundle of non-trivial changes.

**Starting points:**
- Reuse the `TenantResolution.ResolveEffectiveTenantIdAsync` helper in `boilerplateBE/src/modules/Starter.Module.CommentsActivity/Infrastructure/Services/TenantResolution.cs`.
- Apply at the top of `CommentService.EditCommentAsync` and `CommentService.DeleteCommentAsync` — resolve from the persisted comment's `EntityType`/`EntityId`, compare against the stored `TenantId`, log a warning on mismatch, and reject the write if the resolver returns a conflicting tenant.
- Consider also validating the actor's `ICurrentUserService.TenantId` against the resolved value for HTTP callers.
