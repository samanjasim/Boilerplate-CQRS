...
internal sealed class MicrosoftTeamsProvider(
    IHttpClientFactory httpClientFactory,
    ILogger<MicrosoftTeamsProvider> logger) : IIntegrationProvider
{
    public IntegrationType Type => IntegrationType.MicrosoftTeams;
    ...
    public async Task<ProviderDeliveryResult> SendAsync(IntegrationDeliveryRequest request, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            {
                logger.LogInformation("Teams message sent successfully in {Duration}ms", sw.ElapsedMilliseconds);
                return new ProviderDeliveryResult(true, null, null, (int)sw.ElapsedMilliseconds);
            }

            var errorBody = await response.Content.ReadAsStringAsync(ct);
            logger.LogWarning("Teams delivery failed: {StatusCode} - {Body}", response.StatusCode, errorBody);
            return new ProviderDeliveryResult(false, null, $"Teams returned {response.StatusCode}", (int)sw.ElapsedMilliseconds);
        }
        catch (Exception ex)
        {
            sw.Stop();
            logger.LogWarning(ex, "Teams delivery failed");
