...
using Starter.Module.Billing.Domain.Events;
    ...
    ILogger<SyncPlanFeaturesHandler> logger) : INotificationHandler<SubscriptionChangedEvent>
{
    public async Task Handle(SubscriptionChangedEvent notification, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var allFlags = await appContext.Set<FeatureFlag>()
                .AsNoTracking()
            ...
            var existingOverrides = await appContext.Set<TenantFeatureFlag>()
                .IgnoreQueryFilters()
            ...
            foreach (var (key, value) in planFeatures)
            {
                ...
                else
                {
                    ...
                    await appContext.Set<TenantFeatureFlag>().AddAsync(newOverride, cancellationToken);
                }
            }
            ...
            if (toRemove.Count > 0)
                appContext.Set<TenantFeatureFlag>().RemoveRange(toRemove);
